...
class NewsSite {
   ...
   async getRatingCouse(req, res) {
      const { idCourse } = req.params;
      try {
         const allReview = [];
         const ref = admin.firestore().collection("ratings");
         const querySnapshot = await ref.where("idCourse", "==", idCourse).get();

         querySnapshot.forEach((review) => {
            allReview.push({ id: review.id, ...review.data() });
         });

         return res.status(200).json(allReview);
      } catch (error) {
         console.error("Error getting all review:", error);
         throw new Error("Failed to get all review from Firestore");
      }
   }
   ...
   async getAllDataByName(req, res) {
      const { keywords, pageSize, currentPage } = req.params;
      try {
         const allCourses = [];
         const courseRef = admin.firestore().collection("courses");
         let query = courseRef
   ...
   async addCourse(req, res) {
      ...
      try {
         const slug = req.body.title
            ...
            .replace(/[^\w\s]/g, "") // Loại bỏ các ký tự không hợp lệ (giữ lại chữ cái, số, khoảng trắng)
   ...
   async ratingCourse(req, res) {
      try {
         ...
         const courseRef = admin.firestore().collection("ratings");
         await courseRef.doc().set(course);
         await querySnapshot.docs[0].ref.update({ ratingCheck: true });
